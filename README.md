...
- 프로그램이 기억해야 할 것들을 한곳에서 관리합니다.
  - **모든 커밋 목록** — 해시(예: `a1b2c3`)로 커밋을 바로 찾을 수 있게 보관
  - **브랜치별 최신 위치** — `main`, `feature` 같은 브랜치마다 "지금 끝 커밋이 어디인지" 기록
  - **현재 작업 브랜치** — `HEAD`가 가리키는, 지금 커밋이 쌓이는 브랜치
  - **검색용 색인** — 메시지 단어·작성자 이름으로 커밋을 빠르게 찾기 위한 목록
- 사용자가 `COMMIT`, `LOG`, `SEARCH` 등을 입력하면, `main.py`가 이 파일 안의 해당 함수(예: `commit()`, `log()`)를 호출해 실제로 처리합니다.
